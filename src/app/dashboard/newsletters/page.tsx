  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div className="bg-white dark:bg-[#1a1d2e] rounded-xl p-5 shadow-sm">
          <p className="text-sm text-gray-500 dark:text-gray-400">Total Subscribers</p>
          <p className="text-2xl font-bold dark:text-gray-100">{subscribers.length}</p>
        </div>
        <div className="bg-white dark:bg-[#1a1d2e] rounded-xl p-5 shadow-sm">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                filtered.map((s, i) => (
                  <tr key={s._id} className="border-t dark:border-[#2d3148] hover:bg-gray-50 dark:hover:bg-[#252840]/50">
                    <td className="px-4 py-3 text-gray-400">{i + 1}</td>
                    <td className="px-4 py-3 font-medium">{s.email}</td>
                    <td className="px-4 py-3">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${s.isActive ? "bg-green-100 text-green-700" : "bg-gray-100 dark:bg-[#252840] text-gray-500"}`}>
