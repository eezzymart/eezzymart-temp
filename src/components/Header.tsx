...
import {
  ...
  FiUser, FiPhone, FiMapPin, FiChevronRight
} from "react-icons/fi";
...
export default function Header() {
  ...
  return (
    <header className={`sticky top-0 z-50 transition-all duration-300 ${scrolled ? "shadow-lg" : ""}`}>
      ...
      {/* Main Header */}
      <div className={`bg-white border-b border-gray transition-all duration-300 ${scrolled ? "py-2" : "py-3"}`}>
        <div className="container mx-auto px-4 flex items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link href="/dashboard/login" className="hidden sm:flex items-center gap-2 px-3 py-2 rounded-full hover:bg-gray-light transition text-sm text-gray-dark">
              <FiUser size={18} />
              <span className="hidden lg:inline">Account</span>
            </Link>
            <button onClick={openDrawer} className="relative flex items-center gap-2 px-3 py-2 rounded-full hover:bg-gray-light transition group">
      ...
      {/* Mobile Search */}
      {searchOpen && (
        <div className="md:hidden bg-white border-b border-gray px-4 py-3 animate-fade-in">
      ...
      <nav className="bg-gradient-to-r from-[#ff165d] to-[#ff9a00] text-white hidden lg:block">
        <div className="container mx-auto px-4 flex items-center">
          <div className="relative" ref={megaMenuRef}>
            ...
            {megaMenuOpen && (
              <div className="absolute top-full left-0 bg-white text-foreground rounded-b-xl shadow-2xl min-w-[300px] z-50 border border-gray/50 animate-fade-in overflow-hidden">
                    ...
                    return (
                      <div key={cat._id} className="group/cat relative">
                        <Link
                          ...
                          className="flex items-center gap-3 px-5 py-3.5 hover:bg-primary/5 hover:text-primary transition border-b border-gray/30 last:border-0"
                        ...
                        {children.length > 0 && (
                          <div className="hidden group-hover/cat:block absolute left-full top-0 bg-white shadow-2xl rounded-r-xl min-w-[240px] border border-gray/50 overflow-hidden">
                            <div className="px-5 py-3 bg-gray-light/80 border-b border-gray/30">
                              <span className="text-xs font-semibold text-primary uppercase tracking-wider">{cat.name}</span>
                            </div>
                            {children.map((child) => (
                              <Link key={child._id} href={`/shop?category=${child.slug}`} className="block px-5 py-2.5 text-sm hover:bg-primary/5 hover:text-primary transition border-b border-gray/20 last:border-0" onClick={() => setMegaMenuOpen(false)}>
                                {child.name}
                              </Link>
                            ))}
                          </div>
      ...
      {menuOpen && (
        <div className="lg:hidden fixed inset-0 top-0 z-[60] animate-fade-in">
          ...
          <div className="absolute left-0 top-0 bottom-0 w-80 max-w-[85vw] bg-white shadow-2xl overflow-y-auto animate-slide-in-right">
            <div className="flex items-center justify-between p-4 border-b border-gray bg-gray-light">
            ...
            <div className="py-2">
              {navLinks.map((link) => (
                <Link key={link.href} href={link.href}
                  className={`flex items-center justify-between px-5 py-3 text-sm font-medium border-b border-gray/30 transition ${
                    pathname === link.href ? "text-primary bg-primary/5" : "hover:text-primary hover:bg-gray-light"
                  }`} onClick={() => setMenuOpen(false)}>
                  {link.label}
                  <FiChevronRight size={14} className="text-gray-dark" />
                </Link>
              ))}
              <Link href="/order-tracking" className="flex items-center justify-between px-5 py-3 text-sm font-medium border-b border-gray/30 hover:text-primary hover:bg-gray-light transition" onClick={() => setMenuOpen(false)}>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {parentCategories.map((cat) => (
                    <Link key={cat._id} href={`/shop?category=${cat.slug}`}
                      className="flex items-center gap-2 px-3 py-2.5 rounded-lg bg-gray-light hover:bg-primary/10 hover:text-primary transition text-sm"
                      onClick={() => setMenuOpen(false)}>
